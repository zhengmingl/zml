...
class Cx extends TagLib {
    ...
    // import别名 采用文件方式加载(要使用命名空间必须用import) 例如 <load file="__PUBLIC__/__ADMIN__/js/Base.js" />
    public function _load($tag,$content) {
        return $this->_import($tag,$content,true);
    }

    // import别名使用 导入css文件 <css file="__PUBLIC__/__ADMIN__/css/Base.css" />
    public function _css($tag,$content) {
        return $this->_import($tag,$content,true,'css');
    }

    // import别名使用 导入js文件 <js file="__PUBLIC__/__ADMIN__/js/Base.js" />
